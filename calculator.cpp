...
BOOL IsValidNumber(const char* str) {
    ...
    while (*p) {
        if (*p == '.') {
            dotCount++;
            if (dotCount > 1) return FALSE; // มีจุดทศนิยมเกิน 1 จุด
        } else if (!isdigit((unsigned char)*p)) {
            return FALSE; // มีตัวอักษรที่ไม่ใช่ตัวเลข
        }
        p++;
    }
    return TRUE;
...
void PerformCalculation(HWND hwnd, int operation) {
    char buffer1[100], buffer2[100], resultText[100], finalOutput[120];
    ...
    snprintf(resultText, sizeof(resultText), "%g", result);
